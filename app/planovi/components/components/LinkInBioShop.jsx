...
const PRODUCTS = [
  { id: "5k-beginner", name: "Plan za početnike – 5K", price: "€19",
    ...
    icon: <RunnerIcon/>, checkoutUrl: "#", badge: "Najbolje za početak" },
  { id: "half-tri", name: "Triatlon – Half Distance", price: "€49",
    ...
    icon: <TriIcon/>, checkoutUrl: "#", badge: "Popularno" },
  { id: "hybrid-gym", name: "HybridVet – Snaga + izdržljivost", price: "€29",
    ...
    icon: <HybridIcon/>, checkoutUrl: "#", badge: "Novo" },
];

export default function LinkInBioShop() {
  const [email, setEmail] = useState("");
  const [submitted, setSubmitted] = useState(false);
  const handleSubscribe = (e) => { e.preventDefault(); setSubmitted(true); };
    ...
    <div className="min-h-screen bg-neutral-950 text-white">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 py-20 text-center">
          <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-sm mx-auto">
            <Shield className="w-4 h-4" /> Programi provjereni u praksi
          </div>
          ...
          <p className="mt-4 text-neutral-300 max-w-2xl mx-auto">
            Struktura. Dosljednost. Rezultati. Preuzmite plan odmah i krenite danas – bez pretplate.
          </p>
      ...
      {/* Products */}
      <section className="max-w-6xl mx-auto px-4 pb-10 -mt-10">
        <div className="grid md:grid-cols-3 gap-6">
          {PRODUCTS.map((p) => (
            <div key={p.id} className="relative rounded-2xl bg-white/5 backdrop-blur border border-white/10 p-6 shadow-xl">
              {p.badge && (
                <span className="absolute -top-3 left-4 rounded-full bg-emerald-500 text-white text-xs px-3 py-1 shadow">
                  {p.badge}
                </span>
              )}
              ...
              <div className="mt-4 text-3xl font-bold">
                {p.price}<span className="text-base font-normal text-neutral-400"> jednokratno</span>
              </div>
              <ul className="mt-4 space-y-2 text-sm text-neutral-300">
                {p.features.map((f, idx) => (
                  <li key={idx} className="flex items-start gap-2">
                    <Check className="w-4 h-4 mt-0.5" /><span>{f}</span>
                  </li>
                ))}
              </ul>
              <a href={p.checkoutUrl} className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-white text-black font-medium py-3 hover:bg-neutral-200 transition">
                Kupi sada
              </a>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <form onSubmit={handleSubscribe} className="flex w-full md:w-auto gap-2">
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                ...
                className="flex-1 md:w-72 rounded-xl bg-neutral-900 border border-white/10 px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-400" />
              <button className="rounded-xl bg-emerald-500 px-4 py-3 font-medium hover:bg-emerald-400 transition" type="submit">
                Prijavi se
              </button>
            </form>
          </div>
